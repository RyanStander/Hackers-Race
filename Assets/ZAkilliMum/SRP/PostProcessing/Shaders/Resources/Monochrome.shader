Shader "AkilliMum/SRP/PostProcessing/Monochrome"
{
    Properties
    {
        _MainTex("(RGB)", 2D) = "" {}
        ...
        _ColorDark("Color", Color) = (0,0,0,1)
...
UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
 ...
 if (_IsScreenSpace < 0.5)
     ...
     perc = perc - 2.0 * c * perc + c;

     float each = 1.0 / (1.0 - perc);

     float2 v = floor((mod(fragCoord.xy , each) + 1.0) / each);
     ...
     col = lerp(_ColorDark, _ColorLight, sm);
